...
constructor(gl, volume, environmentTexture, options) {
    ...
    this._previewTransferFunctions = [];
    for (let i = 0; i < 8; i++) {
        this._previewTransferFunctions.push(WebGL.createTexture(gl, {
            width  : 2,
            height : 1,
            data   : new Uint8Array([255, 0, 0, 0, 255, 0, 0, 255]),
            wrapS  : gl.CLAMP_TO_EDGE,
...
render(/*i = "tf", x = 0, y = 0, w = this._bufferSize, h = this._bufferSize*/) {
    ...
    this._frameBuffer.use(/*x, y, w, h*/);
    this._generateFrame();

    this._accumulationBuffer.use(/*x, y, w, h*/);
    this._integrateFrame(/* i */);
    this._accumulationBuffer.swap();

    this._renderBuffer.use(/* x, y, w, h */);
    this._renderFrame();
}

/*
render(x = 0, y = 0, w = this._bufferSize, h = this._bufferSize, i = "tf") {
    // TODO: put the following logic in VAO
    const gl = this._gl;
    gl.bindBuffer(gl.ARRAY_BUFFER, this._clipQuad);
    gl.enableVertexAttribArray(0); // position always bound to attribute 0
...
setPreviewTransferFunction(transferFunction) {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, transferFunction.width, transferFunction.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, transferFunction.data);
}
